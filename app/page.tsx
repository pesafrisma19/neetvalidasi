...
export default function Home() {
    return (
        <div className="flex flex-col min-h-screen bg-ios-bg dark:bg-black p-6 pt-12 relative overflow-hidden transition-colors duration-300">
            {/* Background Blobs */}
            ...
            <div className="mb-10 relative z-10 flex items-center justify-between">
                <div>
                    <h1 className="text-[34px] font-bold tracking-tight text-black dark:text-white mb-1">
                        Validasi
                    </h1>
            ...
            {/* Menu Grid */}
            <div className="grid gap-5 relative z-10 h-full grid-cols-1 md:grid-cols-2">
                {MENU_ITEMS.map((item, index) => (
                    <Link href={item.href} key={item.href} className="block group">
                        <IOSCard className="flex items-center gap-5 transition-transform duration-300 active:scale-95 hover:bg-white/80 dark:hover:bg-gray-800/80 border-white/40 dark:border-white/10 dark:bg-gray-900/50">
                            <div className={`w-14 h-14 rounded-2xl ${item.color} flex items-center justify-center shadow-lg shadow-${item.color}/30 text-white shrink-0`}>
                                <item.icon className="w-7 h-7" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className="text-[20px] font-semibold text-black dark:text-white leading-tight truncate">
                                    {item.title}
                                </h3>
                                <p className="text-[15px] text-gray-400 font-medium mt-0.5 truncate">
                                    {item.description}
                                </p>
                            </div>
            ...
            <div className="mt-auto py-6 text-center text-xs text-gray-400 font-medium md:hidden">
